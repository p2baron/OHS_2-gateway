          ...
          if (nodeIndex != DUMMY_NO_VALUE ) {
            ...
            if (nodeIndex == DUMMY_NO_VALUE) {
              ...
              if (node[nodeIndex].type == 'K') {
                ...
                if (groupNum < ALARM_GROUPS) {
                  if (GET_GROUP_ALARM(group[groupNum].setting) == 0) {
                    if (GET_GROUP_WAIT_AUTH(group[groupNum].setting)) {
                      pushNodeCmd(inMsg->address, NODE_CMD_AUTH_1);
                    } else {
                      if (GET_GROUP_ARMED(group[groupNum].setting)) {
                        pushNodeCmd(inMsg->address, NODE_CMD_ARMED_AWAY);
                      } else {
                        if (group[groupNum].armDelay > 0) {
                          pushNodeCmd(inMsg->address, NODE_CMD_ARMING);
                        } else {
                          pushNodeCmd(inMsg->address, NODE_CMD_DISARM);
                        }
                      }
                    }
                  } else {
                    pushNodeCmd(inMsg->address, NODE_CMD_ALARM);
                  }
                }
              } else {
